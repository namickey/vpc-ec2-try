...
- インターネットゲートウェイを１つ新規作成する
  - VPC内に作成するEC2インスタンスに対して、外部からアクセスできるようにしたい
  - インターネットゲートウェイは`ルートテーブル`への設定が必要
  - インターネットゲートウェイが設定されたルートテーブルを使用するサブネットを`パブリックサブネット`と呼ぶ。そうではないものは`プライベートサブネット`と呼ぶ
- 作成後に表示されるダイアログに従って、今回作成した`VPC`に`igw`をアタッチする
  ※igw=インターネットゲートウェイ

## パブリックサブネット用のルートテーブルの作成とルート編集

- パブリックサブネット用のルートテーブルを新規作成する
  - 新規作成せずに、VPCのデフォルトのルートテーブルを使用しても良い。
- `VPC`のルートテーブルに`igw`を追加設定する
  - 送信先は`0.0.0.0/0`を設定する
  - ターゲットは今回作成した`igw`を設定する

> [!TIP]
> - `PIBLIC`と`PRIVATE`を分けるならば、ルートテーブルは２つ必要で、サブネットへのルートテーブルの関連付けが必要。
...
- 作成したサブネットを開き、作成したルートテーブルを関連付ける。
  - VPCデフォルトのルートテーブルがサブネットに設定されているため、新規作成したパブリックサブネット用のルートテーブルを使う場合には、関連付けの変更が必要。
